...
function Exec.createTaskSequenceList()
...
function Exec.createTaskSequence(...)
...
function Exec.createTask(delay)
	local task = {};
...
function Exec.scheduleTask(task, delay)
	task.delay = delay;
	local sequence = Exec.createTaskSequence(task);
	Exec.ats.addSequence(sequence);
end