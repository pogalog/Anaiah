-- Inventory Item
require( "game.item.weapon" );
require( "game.item.register" );
...
-- item types
Game.ITEM_HEALING = 0;
Game.ITEM_RECOVERY = 1;
Game.ITEM_BUFF = 2;
Game.ITEM_DEBUFF = 3;
Game.ITEM_STAT = 4;
Game.ITEM_TREASURE = 5;

function Game.createItem( id, itemType, name )
	local item = {};
	item.id = id;
	item.type = itemType;
	item.name = name;
	...
	function item.copy()
		local cp = Game.createItem( item.id );
		cp.name = item.name;
...
function Game.readItemsFromDisk()
	...
	for ind, line in pairs( ln ) do
		...
		local item = Game.createItem( tonumber( info[1] ), info[2] );
...
Weapons = {};
-- read item info from items.txt
function Game.readWeaponsFromDisk()
	local filename = "./resource/item/weapons.txt";
	local ln = linesFromFile( filename );
	for ind, line in pairs( ln ) do
		local info = prepareInfo( line );
		local weapon = Game.createWeapon( tonumber( info[1] ) );
		weapon.name = info[2];
