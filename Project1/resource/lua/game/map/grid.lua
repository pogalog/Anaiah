...
-- grid
function Game.createGrid( map, xsize, zsize )
	local grid = {};
	local rows = {};
	grid.map = map;
	grid.size = Vec2_new( xsize, zsize );
	grid.pathFindTargets = createList();
	for i = 0, zsize-1 do
		rows[i+1] = Game.Grid.createRow( i, xsize );
	end
	grid.rows = rows;
	grid.desc = "MapGrid";
	grid.color = Vec3_new( 1, 1, 1 );
	
	-- axis vectors
	grid.directions = createList();
	grid.directions.add( Vec2_new( 1, 1 ) );
	grid.directions.add( Vec2_new( 0, 1 ) );
	grid.directions.add( Vec2_new( -1, 0 ) );
	grid.directions.add( Vec2_new( -1, -1 ) );
	grid.directions.add( Vec2_new( 0, -1 ) );
	grid.directions.add( Vec2_new( 1, 0 ) );
	...
	-- FUNCTIONS
	function grid.generateModel()
		grid.model = Geom.createModel( Model_buildGrid( GameInstance ) );
		grid.model.setUniform( "vec3 color", grid.color );
	end
	
	function grid.setShader( shader )
		grid.shader = shader;
		Model_setShader( grid.model.userdata, shader.userdata );
	end
	...
	function grid.getDirectionIndexFromVector( fv_unit )
		local pi3 = math.pi / 3.0;
		local angle = math.atan2( fv_unit.z, fv_unit.x ) + pi3;
		local div = math.round( angle / pi3 );
		return (6 - div) % 6;
	end
	
	
	function grid.getGridDirectionFromWorldVector( wv )
		local wv_unit = Vec3_getUnit( wv );
		local di = grid.getDirectionIndexFromVector( wv_unit );
		return grid.directions.get( di + 1 );
	end
	
	function grid.buildNeighbors()
		for k, row in pairs( grid.rows ) do
			for k1, tile in pairs( row.tiles ) do
				grid.buildNeighborForTile( tile );
			end
		end
	end
	
	function grid.buildNeighborsForTiles( tiles )
	...
	function grid.buildNeighborForTile( tile )
  ...
  function grid.clearCustomFlowMarkings()
	...
	function grid.getAttackRange( unit )
		...
		for i = 1, moveRange.length() do
			...
			for j = 1, shape.length() do
				...
				local realAddress = Vec2_new( localAddress.x+center.x, localAddress.y+center.y );
	...
	function grid.getSingleAttackRange( unit, tile )
		...
		local coexistentShapes = createList();
		
		for j = 1, shape.length() do
			local localAddress = shape.get(j).address;
			local realAddress = Vec2_new( localAddress.x + center.x, localAddress.y + center.y );
			local atkTile = grid.getTileAtAddress( realAddress.x, realAddress.y );
			if( atkTile ~= nil and atkTile.coexists() ) then
				-- find the offset of this tile from the center
				local offset = Vec2_sub( atkTile.address, center );
				
				-- use that offset to create a new shape with shifted coordinates
				local shifted = shape.createShiftedCopy( offset );
				shifted.home = atkTile.coexist.address;
				coexistentShapes.add( shifted );
			end
			
			if( atkTile ~= nil and atkTile.exists and atkRange.contains( atkTile ) == false ) then
				atkRange.add( atkTile );
				if( atkTile.coexist ~= nil ) then
					atkRange.add( atkTile.coexist );
				end
			end
		end
		
		-- add tiles from the coexistent shapes
		for i = 1, coexistentShapes.length() do
			local copy = coexistentShapes.get(i);
			for j = 1, copy.length() do
				local localAddress = copy.get(j).address;
				local realAddress = Vec2_new( localAddress.x + copy.home.x, localAddress.y + copy.home.y );
				local atkTile = grid.getTileAtAddress( realAddress.x, realAddress.y );
				if( atkTile ~= nil and atkTile.exists and atkRange.contains( atkTile ) == false ) then
					atkRange.add( atkTile );
				end
	...
	function grid.isTileWithinRange( tile0, tile1, distance, ignoreUnits )
		local range = grid.getRange( tile0, distance, ignoreUnits );
		return range.contains( tile1 );
	end
	...
	function grid.generateAttackShape( unit )
		...
		function shape.shift( offset )
			for i = 1, shape.length() do
				local tile = shape.get(i);
				Vec2_subLocal( tile.address, offset );
			end
		end
		
		
		function shape.createShiftedCopy( offset )
			local copy = grid.generateAttackShape( unit );
			copy.shift( offset );
			copy.offset = offset;
	...
	function grid.findPath()
		...
		-- add coexisting tiles to pathfind target list		
		for i = 1, grid.pathFindTargets.length() do
			local target = grid.pathFindTargets.get(i);
			if( target.coexist ~= nil ) then
				local ct = target.coexist;
				traversal.add( ct );
				ct.pathValCost = 0;
				ct.goal = ct;
			end
		...
		while( traversal.length() > 0 ) do
			for i = 1, traversal.length() do
				...
				for j = 0, 5 do
					...
					-- CHECK THE REST OF THIS FUNCTION FOR LOGICAL ERRORS!!!
--					if( neighbor ~= nil and neighbor.coexist ~= nil ) then neighbor = neighbor.coexist; end
					...
					if( neighbor ~= nil and neighbor.isPathable() ) then
						...
						local dir = j;
						-- TODO this calculuation must be updated in order to pathfind with manually specified neighbors
						local undir = (dir + 3) % 6;
						
--						print( "Testing: " .. Vec2_tostring( tile.address ) .. "->" .. Vec2_tostring( neighbor.address ) .. "(" .. myCost .. "->" .. neighborCost .. ")" );
...
--							print( "Convert! Best tile of " .. Vec2_tostring( neighbor.address ) .. " is " .. Vec2_tostring( tile.address ) );
							if( neighbor.isNeighbor( tile ) == false ) then
								neighbor.bestTile = tile.coexist;
							end
							neighbor.bestTile2 = nil;
							neighbor.goal = tile.goal;
							if( neighbor.coexist ~= nil ) then
								local nc = neighbor.coexist;
								nc.pathValCost = myCost + mvPenalty + 1;
								nc.pathValDir = undir;
								nc.bestTile = tile;
								if( nc.isNeighbor( tile ) == false ) then
									nc.bestTile = tile.coexist;
								end
								nc.bestTile2 = nil;
								nc.goal = tile.goal;
							end
						    ...
						    if( neighbor.occupant == nil ) then
								if( neighbor.coexist ~= nil ) then
									assignment.add( neighbor.coexist );
								else
							  		assignment.add( neighbor );
								end
						    end
...
--								end
--							end
--							local otherDist = Vec2_hexDistanceTo( neighbor.goal.address, neighbor.bestTile.address );
--							local myDist = Vec2_hexDistanceTo( tile.address, tile.goal.address );
...
-- row
function Game.Grid.createRow( zAddress, size )
	local row = {};
	local tiles = {};
	for i = 0, size-1 do
